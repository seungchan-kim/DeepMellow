...
class Network(object):
  ...
  def build_output_ops(self, input_layer, network_output_type, 
    ...
    self.omega = tf.placeholder('float32',[],'omega')
    self.outputs_with_idx = tf.gather_nd(self.outputs, self.outputs_idx)
    self.actions = tf.argmax(self.outputs, axis=1)

    #mellowmax

    self.mellowmax_outputs = self.mellowmax(self.outputs, self.omega, n=output_size, l=32, reduction_indices=1)

  def mellowmax(self, input_tensor, w, n, l=32, reduction_indices=1):
  ...
  def calc_mellowmax_outputs(self, observation, w):
    #print(self.outputs.eval({self.inputs:observation},session=self.sess))
    #print(self.mellowmax_outputs.eval({self.inputs:observation},session=self.sess))
    return self.mellowmax_outputs.eval({self.inputs: observation, self.omega: w}, session=self.sess)
