...
num_episodes = 1200		# number of episodes: originally 5000 
max_steps_ep = 10000	# default max number of steps per episode (unless env has a lower hardcoded limit)
slow_target_burnin = 1000		# number of steps where slow target weights are tied to current network weights
update_slow_target_every = 1	# number of steps to use slow target as target before updating it to latest weights
...
iterNum = sys.argv[1]
w = int(sys.argv[2])
file1 = open("csv/score_mellow"+str(w)+"_"+ str(iterNum)+".csv",'w')
...
def mellowmax(input_tensor, w, n, l, axis=None):
        c = tf.scalar_mul(w, input_tensor)
        a = tf.reduce_max(c,axis)
        a_ = tf.reshape(a,[l,1])
        y = tf.log(tf.reduce_sum(tf.exp(tf.subtract(c,a_)),axis))+a
        return tf.scalar_mul(1/w,np.log(1/n)+y)
	#return tf.scalar_mul(1/w,tf.log(tf.reduce_mean(tf.exp(tf.scalar_mul(w,input_tensor)),axis)))
...
targets = reward_ph + is_not_terminal_ph * gamma * mellowmax(slow_target_action_values, w, n_actions, minibatch_size, axis=1)
	#mellowmax(slow_target_action_values, 20, 1)
	#tf.reduce_max(slow_target_action_values,axis=1)
...
for ep in range(num_episodes):
	...
	file1.write(str(total_reward)+','+str(ep)+',\n')
